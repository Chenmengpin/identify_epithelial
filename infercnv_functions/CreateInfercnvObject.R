CreateInfercnvObject <- function (raw_counts_matrix, gene_order_file, annotations_file,
    ref_group_names, delim = "\t", max_cells_per_group = NULL,
    min_max_counts_per_cell = NULL, chr_exclude = c("chrX", "chrY",
        "chrM"))
{
    # check if raw matrix is character class:
    ...
    if (!is.null(min_max_counts_per_cell)) {
        ...
        cs = colSums(raw.data)
        cells.keep <- which(cs >= min_counts_per_cell & cs <=
            max_counts_per_cell)
        ...
        ref_group_names <- ref_group_names[ref_group_names %in%
            unique(input_classifications[, 1])]
        if (!all.equal(ref_group_names, orig_ref_group_names)) {
            flog.warn(sprintf("-warning, at least one reference group has been removed due to cells lacking: %s",
                orig_ref_group_names[!orig_ref_group_names %in%
                  ref_group_names]))
        }
    }
    if (!is.null(max_cells_per_group)) {
        ...
        for (grp in names(grps)) {
            ...
            if (dim(df)[1] > max_cells_per_group) {
                ...
                grps[[grp]] = df[sample(1:dim(df)[1], max_cells_per_group),
                  , drop = FALSE]
            }
        }
