...
# makes normal calls based upon automatic cluster detection-based thresholds of
# per-cell CNA values and correlation with top 5% cancer cells

### inputs ###

# seurat object
# ordered gene co-ordinate reference

######

### final outputs ###

# inferCNV heatmap with:
# nUMI
# nGene
# CNA values
# normal calls
# CNV-associated oncogenes
# METABRIC subtype freqs

# cancer-only inferCNV heatmap with:
# nUMI
# nGene
# CNA values
# SNP CNV call heatmap
...
print(paste0("subset data? ", subset_data))
subproject_name <- args[4]
print(paste0("subproject name is ", subproject_name))
throw_unneeded <- as.logical(args[5])
print(paste0("throw unneeded cells? " throw_unneeded))
...
new_seurat_dir <- paste0(seurat_path, "seurat_", sample_name, 
  "/Output/Rdata/normal_call/")
  system(paste0("mkdir -p ", new_seurat_dir))

out_dir <- paste0("InferCNV/normal_call/")
