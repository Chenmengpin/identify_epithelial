create_group_annotation_groupplot <- function(df, metadata, metastases) {
  ...
  type_annotation_df$group[grep("CID", type_annotation_df$group)] <- "Primary"
  type_annotation_df$group[grep("[0-9]N", type_annotation_df$group)] <- "Normal"
  type_annotation_df$group[sample_annotation_df$group %in% metastases] <- "Metastasis"
  ...
  all_df <- rbind(result_df[grep("Normal", result_df$group.x),],
    result_df[grep("Primary", result_df$group.x),])
  all_df <- rbind(all_df, result_df[grep("Metastasis", result_df$group.x),])
  ...
  group_annotation <- Heatmap(df_list$group_annotation_df, col = cluster_cols, 
   ...
   col_palette2 <- c("#9ECAE1", "#3F007D", "#67000D", "#FDD0A2", "#08519C",
    ...
    "#7F2704", "#DADAEB", "#FC9272", "#BCBDDC", extra_colours)
  ...
  col_palette3 <- c("#b2182b", 
            ...
            "#8ff331","#270e26", extra_colours, col_palette2)
