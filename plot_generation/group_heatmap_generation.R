...
#for a in ${annots[@]}; do
#  for s in ${samples[@]}; do
#    mkdir -p /share/ScratchGeneral/jamtor/projects/single_cell/identify_epithelial/scripts/logs/$s.annot.$a.group/
#    qsub -wd /share/ScratchGeneral/jamtor/projects/single_cell/identify_epithelial/scripts/logs/$s.annot.$a.group/ -pe smp 16 -N $s.annot.$a.group -b y -j y -V -P TumourProgression "${R} CMD BATCH  --no-save '--args $s $a' /share/ScratchGeneral/jamtor/projects/single_cell/identify_epithelial/scripts/group_heatmap_generation.R"
#  done;
#done;
...
temp_sample <- "normals"
include_annotations <- as.logical("TRUE")
...
if (include_annotations) {
  ...
  out_path <- paste0(seurat_path, "brca_mini_atlas/", 
    paste(sample_ids, collapse = "_"), "_no_annotations/")
}
...
for (i in 1:length(sample_ids)) {
  ...
  infercnv_output_filename <- list.files(
    ...
    sample_ids[i], 
    "/Output/InferCNV/supervised_clustering"), pattern = "denoised.observations.txt",
    full.names = T
  )
...
###
#save.image(paste0(out_path, "temp_image_2.RData"))
###
...
if (include_annotations) {
  ...
  if (mets != "none") {
    ...
    group_annotation$group_annotation + final_heatmap + QC_annotation$nUMI_annotation +
    ...
    if (missing_genes_colour != "white") {
      ...
      pushViewport(viewport(x=x_coord + 0.848, y=0.41, width = 0.1, height = 0.1, just = "bottom"))
      #grid.draw(lollipop)
      grid.text("nUMI", rot=55)
      popViewport()
  
      pushViewport(viewport(x=x_coord + 0.866, y=0.41, width = 0.1, height = 0.1, just = "bottom"))
      #grid.draw(lollipop)
    ...
    # (0.00604 units per character):
    longest_cluster_name <- max(nchar(unique(as.character(group_annotation$group_annotation_df$group))))
    x_coord <- longest_cluster_name*0.0036
    ...
    grid.newpage()
      # plot Normal subtype:
      pushViewport(viewport(x = x_coord+0.005, y = 0.090,
                            width = 0.852+0.005, height = 0.08, just = c("left", "top")))
      grid.draw(metabric_plots[[5]])
      popViewport()
      
      # plot Basal subtype:
      pushViewport(viewport(x = x_coord+0.008, y = 0.169,
                            width = 0.852+0.002, height = 0.08, just = c("left", "top")))
      grid.draw(metabric_plots[[4]])
      popViewport()
      
      # plot Her2 subtype:
      pushViewport(viewport(x = x_coord+0.01, y = 0.251, 
                            width = 0.852, height = 0.08, just = c("left", "top")))
      grid.draw(metabric_plots[[3]])
      popViewport()
      
      # plot LumB subtype:
      pushViewport(viewport(x = x_coord+0.007, y = 0.333, 
                            width = 0.852+0.003, height = 0.08, just = c("left", "top")))
      grid.draw(metabric_plots[[2]])
      popViewport()
      ...
      pushViewport(viewport(x=x_coord + 0.868, y=0.41, width = 0.1, height = 0.1, just = "bottom"))
      #grid.draw(lollipop)
      grid.text("nUMI", rot=55)
      popViewport()
  
      pushViewport(viewport(x=x_coord + 0.886, y=0.41, width = 0.1, height = 0.1, just = "bottom"))
      #grid.draw(lollipop)
      grid.text("nGene", rot=55)
      popViewport()
  ...
  if (missing_genes_colour != "white") {
    pdf(paste0(out_path, "final_infercnv_heatmap_missing_genes_coloured_non_annotated.pdf"), 
        height = 9, width = 14)
  } else {
      pdf(paste0(out_path, "final_infercnv_heatmap_non_annotated.pdf"), 
        height = 9, width = 14)
  }
