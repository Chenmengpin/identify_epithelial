...
######
temp_wd <- "/share/ScratchGeneral/jamtor/projects/single_cell/identify_epithelial/results/seurat/brca_mini_atlas_030719/seurat_CID4463_sub"
# 06 SEURAT PARAMS FILE
#temp_params_file <- temp_args[6]
temp_params_file <- 
  "/share/ScratchGeneral/jamtor/projects/single_cell/identify_epithelial/scripts/seurat_v3_individual_processing_normals_included/seurat_V3_HPC_params_file.csv"
# 07 INFERCNV REFERENCE DIRECTORY
#temp_infercnv_reference_dir <- temp_args[7]
temp_infercnv_reference_dir <- 
  "/share/ScratchGeneral/jamtor/projects/single_cell/identify_epithelial/refs/"
# 07 INFERCNV FUNCTION DIRECTORY
#temp_infercnv_function_dir <- temp_args[8]
temp_infercnv_function_dir <- 
  "/share/ScratchGeneral/jamtor/projects/single_cell/identify_epithelial/scripts/seurat_v3_individual_processing_normals_included/functions/"
...
### InferCNV functions ###
prepare_infercnv_metadata <- dget(paste0(temp_infercnv_function_dir, 
  "prepare_infercnv_metadata.R"))
create_infercnv_object <- dget(paste0(temp_infercnv_function_dir, 
  "create_infercnv_object.R"))
run_infercnv <- dget(paste0(temp_infercnv_function_dir, 
  "run_infercnv.R"))
gg_color_hue <- dget(paste0(temp_infercnv_function_dir, 
  "gg_color_hue.R"))
create_group_annotation <- dget(paste0(temp_infercnv_function_dir, 
  "create_group_annotation.R"))
fetch_chromosome_boundaries <- dget(paste0(temp_infercnv_function_dir, 
  "fetch_chromosome_boundaries.R"))
create_QC_annotation <- dget(paste0(temp_infercnv_function_dir, 
  "create_QC_annotation.R"))
annotate_PAM50_CNV <- dget(paste0(temp_infercnv_function_dir, 
  "annotate_PAM50_CNV.R"))
create_QC_annotation <- dget(paste0(temp_infercnv_function_dir, 
  "create_QC_annotation.R"))
create_CNV_genes_annotation <- dget(paste0(temp_infercnv_function_dir, 
...
if (temp_run_infercnv) {
  ...
  if (temp_half_stromal) {
    system(paste0("mkdir -p Output/InferCNV/half_stromal/input_files/"))
    write.table(infercnv_metadata$metadata, 
    "Output/InferCNV/half_stromal/input_files/metadata.txt", 
    quote=F, sep="\t", col.names=F, row.names=F)
    write.table(infercnv_metadata$number_per_group, 
      "Output/InferCNV/half_stromal/input_files/number_per_group.txt", 
  ...
  print(paste0("Normals are: ", paste(normals, collapse=", ")))
  ...
  ######
  if (temp_half_stromal) {
    if (!file.exists("Output/InferCNV/half_stromal/input_files/raw_counts_matrix.txt")) {
      ...
      "Output/InferCNV/half_stromal/input_files/raw_counts_matrix.txt", quote=F,
      sep="\t", col.names=T, row.names=T)
      print("Creating inferCNV object...")
      raw_path <- paste0(temp_wd, "/Output/InferCNV/half_stromal/input_files/raw_counts_matrix.txt")
      annotation_path <- paste0(temp_wd, "/Output/InferCNV/half_stromal/input_files/metadata.txt")
      gene_path <- paste0(temp_infercnv_reference_dir, "infercnv_gene_order.txt")
      out_dir <- "Output/InferCNV/half_stromal/supervised_clustering/"
  ...
  infercnv_output <- run_infercnv(initial_infercnv_object, 5, out_dir, temp_infercnv_cutoff, 
  ...
  if (temp_run_HMM) {
    infercnv_observations <- read.table(paste0(out_dir,  
      "/infercnv.14_HMM_predHMMi6.hmm_mode-samples.repr_intensities.observations.txt"), fill=T)
  } else {
    infercnv_observations <- read.table(paste0(out_dir, 
  ...
  chr_data <- fetch_chromosome_boundaries(heatmap_df, temp_infercnv_reference_dir)
  ...
  ######
  # create SNP array CNV annotation:
  SNP_CNVs <- read.table("Output/SNP_CNVs/SNP_CNV_profile.txt", header=T, 
    as.is=T)
  create_SNP_CNV_annotation <- function(heatmap_df, CNV_df, chr_ends, 
    chr_lengths) {

    # append meta_cnv info onto observation_df column to put it in order:
    temp <- as.data.frame(t(heatmap_df[1,]))
    temp$symbol <- rownames(temp)
    df <- merge(temp, CNV_df, by="symbol", all.x = T)
    df <- df[,grep("CNV|symbol", colnames(df))]

    # remove duplicates and NA values from cnv_df:
    df <- df[-which(duplicated(df$symbol)),]
    df$Tumour_CNV_Value[is.na(df$Tumour_CNV_Value)] <- 2
    df$Blood_CNV_Value[is.na(df$Blood_CNV_Value)] <- 2

    # transform scores from allele frequency into CNV levels:
    old_scores <- c(0, 1, 2, 3, 4)
    new_scores <- c(-2, -1, 0, 1, 2)
    for (s in 1:length(new_scores)) {
      df$Tumour_CNV_Value[df$Tumour_CNV_Value == old_scores[s]] <- 
        new_scores[s]
      df$Blood_CNV_Value[df$Blood_CNV_Value == old_scores[s]] <- 
        new_scores[s]
    }

    # split into blood and tumour dfs:
    df_list <- list(subset(df, select = -Tumour_CNV_Value), 
      subset(df, select = -Blood_CNV_Value))
    names(df_list) <- c("blood_array", "tumour_array")

    # split into loss and gain CNV columns:
    k=1
    plot_list <- lapply(df_list, function(x) {
      colnames(x)[grep("CNV", colnames(x))] <- "CNV"
      x$Gain <- x$CNV
      x$Gain[x$Gain < 0] <- 0
      x$Loss <- x$CNV
      x$Loss[x$Loss > 0] <- 0
      x <- subset(x, select=-CNV)

      # create area plot of CNVs:
      # adjust plot df:
      x$BPcum <- seq(1, nrow(x))
      m_df <- melt(x, id.vars = c("symbol", "BPcum"))
      m_df <- m_df[order(m_df$BPcum),]

      # prepare df for labelling:
      colnames(m_df) <- c("Gene", "Genomic_Region", "CNV_type", "CNV_value")
      # define colours:
      cols <- gg_color_hue(2)

      # create area plot:
      p <- ggplot(m_df, aes(x=Genomic_Region, y=CNV_value))
      p <- p + geom_area(aes(color=factor(CNV_type, levels = c("Gain", "Loss"))))
      p <- p + scale_fill_manual(c(cols[1], cols[2]))
      p <- p + scale_x_continuous(label = names( c(chr_ends, "") ), breaks = c(0, chr_ends),
                                  expand = c(0,0))
      p <- p + theme_bw()
      p <- p + scale_y_continuous(name=paste0(names(df_list)[k]), breaks = c(-2, 0, 2), 
                                  limits = c(-1, 1))
      p <- p + theme(axis.text.x = element_blank(),
                     axis.ticks.x = element_blank(),
                     axis.title.x = element_blank(),
    ...
    names(plot_list) <- names(df_list)
    return(plot_list)
  }
  SNP_CNV_annotation <- create_SNP_CNV_annotation(heatmap_df, SNP_CNVs, chr_data$ends, 
    chr_data$lengths)
  ######
  ...
  if (temp_run_HMM) {
    ...
    pdf("Output/InferCNV/supervised_clustering/infercnv_final_prediction_heatmap.pdf", 
    height = 10, width = 11)
...
#      grid.draw(metabric_plots[[1]])
      ...
      # plot tumour SNP CNV annotation:
      pushViewport(viewport(x = 0.102, y = 0.551, 
                            width = 0.9975-0.101, height = 0.08, just = c("left", "top")))
      grid.draw(SNP_CNV_annotation[[2]])
      popViewport()
      
      # plot blood SNP CNV annotation:
      pushViewport(viewport(x = 0.102, y = 0.611, 
                            width = 0.9975-0.101, height = 0.08, just = c("left", "top")))
      grid.draw(SNP_CNV_annotation[[1]])
      popViewport()

#      popViewport()
#      pushViewport(viewport(x = 0, y = 0.6, 
#                            width = 0.99, height = 0.4, just = c("left", "bottom")))
...
#      })
    ...
    height = 10, width = 11)
      ...
      # plot tumour SNP CNV annotation:
      pushViewport(viewport(x = 0.102, y = 0.511, 
                            width = 0.9975-0.101, height = 0.08, just = c("left", "top")))
      grid.draw(SNP_CNV_annotation[[2]])
      popViewport()
