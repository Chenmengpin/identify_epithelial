...
if (temp_export_normalised_matrix) {
  ...
  write.table(
    temp_normalised.data.frame,
    file = "Output/Matrices_and_metadata/normalized_expression_matrix.txt",
    sep = "\t",
    col.names = T
  )
  ...
  #Associated Metadata
  write.table(
    cbind(seurat_10X@meta.data,
          Cluster_ID = seurat_10X@ident),
    file = "Output/Matrices_and_metadata/normalized_expression_metadata.txt",
    sep = "\t",
    col.names = T,
    row.names = T
  )
...
if (temp_export_raw_UMI_matrix) {
  ...
  write.table(
    temp_raw.data.frame,
    file = "Output/Matrices_and_metadata/raw_UMI_count_matrix.txt",
    sep = "\t",
    col.names = T
  )
  ...
  write.table(
    cbind(seurat_10X@meta.data,
          Cluster_ID = seurat_10X@ident),
    file = "Output/Matrices_and_metadata/raw_UMI_count_metadata.txt",
    sep = "\t",
    col.names = T,
    row.names = T
  )
