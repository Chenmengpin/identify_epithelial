...
if (temp_run_infercnv) {
  ...
  # if no epithelial or CAF clusters present, abort run:
...
#    print("No epithelial or CAF clusters detected, aborting...")
#    # touch file as indicator that there are no epithelial or CAF clusters:
#    system("touch Output/InferCNV/run_aborted_as_no_epithelial_or_CAF_clusters")
#  } else {
...
#    }
    epi_cluster <- "6"
    temp_levels <- levels(Idents(seurat_10X))
    temp_levels[grep(epi_cluster, temp_levels)] <- paste0(
      "Epithelial_", gsub("^.*_", "", temp_levels[grep(epi_cluster, temp_levels)])
    )
    levels(Idents(seurat_10X)) <- temp_levels

    print(paste0("Idents are: ", levels(Idents(seurat_10X))))

    infercnv_metadata$metadata$cell_type <- as.character(infercnv_metadata$metadata$cell_type)
    infercnv_metadata$metadata$cell_type[grep(
      epi_cluster, infercnv_metadata$metadata$cell_type
    )] <- paste0(
        "Epithelial_", gsub(
          "^.*_", "", infercnv_metadata$metadata$cell_type[grep(
            epi_cluster, as.character(infercnv_metadata$metadata$cell_type)
          )]
        )
      )
    ...
    if (temp_run_HMM) {
      ...
      system("rm Output/InferCNV/supervised_clustering/*14*")
    }    
  }
}
