...
#sample_ids=( CID3586 CID3921 CID3941 CID3948 CID3963 )
...
for sample in ${sample_ids[@]}; do
  ...
  log_dir="$seurat_path/seurat_$id/Output/Logs/$id.combined_heatmap/"
  mkdir -p $log_dir
  echo "Logs are in $log_dir"
  qsub -wd $log_dir -pe smp $ncores -N CID$short_id.cplot -b y -j y -V -P TumourProgression \
  "$R_dir/R CMD BATCH  --no-save '--args $subproject_name $id $subset_data $$draw_bulk_annotations' $script_dir/4.plot_individual_combined_heatmap.R"
