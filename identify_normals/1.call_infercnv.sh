...
for sample in ${sample_ids[@]}; do
  ...
  qsub -wd $log_dir -pe smp $ncores -N c$ncores.$short_id -b y -j y -V -P TumourProgression \
    "${R} CMD BATCH  --no-save '--args $subproject_name $id $subset_data $ncores $draw_bulk_annotations' $script_dir/2.infercnv.R"
